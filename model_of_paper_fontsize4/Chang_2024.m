...
% Collision Alarm
a=load('D:\matlabWorkspace\model_of_paper_fontsize4\results\result_2024.txt');
% plot(a(:,1),a(:,2),'-r','LineWidth',6);
% hold on;

a_size = size(a); % a的尺寸
a_row = a_size(1, 1);
% 绘制垂直于x轴的线段(x1,0.5)至(x1,y1)，使用line([x1 x1], [0.5 y1])画线段;
for i = 1 : 1 : a_row
    x1 = i;
    y1 = a(i, 2);
    %
    line([x1 x1], [0.5 y1], 'linestyle', '-', 'Color', 'r', 'LineWidth', 3); % [1 0.5 0.8]即为粉红色
    
end


% 绘制平行于x轴的线段(0,0.78)至(x2,0.78)，使用line([0 x2], [0.78 0.78])画线段;
x2 = a_row;
line([0 x2], [0.78 0.78], 'linestyle', '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 5); % [0.5 0.5 0.5]即为灰色
...
set(gca, 'position', [0.114, 0.178, 0.876, 0.763]);   % 修改第三处
