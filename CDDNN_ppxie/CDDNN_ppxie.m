...
% LGMD Neuron
% a=load('E:\Awodebiancheng\PyCharm\ScientificResearch\CDDNN_ppxie\CDDNN\results\kf.txt');
a=load('.\results\kf.txt');
plot(a(:,1),a(:,2),'-g','LineWidth',3);
hold on;

% Threshold Level
% a=load('E:\Awodebiancheng\PyCharm\ScientificResearch\CDDNN_ppxie\CDDNN\results\Ts.txt');
a=load('.\results\Ts.txt');
plot(a(:,1),a(:,2),'--b','LineWidth',3);
hold on;

% Collision Alarm
% a=load('E:\Awodebiancheng\PyCharm\ScientificResearch\CDDNN_ppxie\CDDNN\results\warning.txt');
...
legend(' BVT Neuron ', ' Threshold Level ',' Collision Alarm ');

set(gca,'linewidth',1,'fontsize',35,'fontname','Times');

% y坐标范围为[0, 1.05]，但是刻度上并不显示1.05这个刻度，只显示[0, 1.0]这些刻度
