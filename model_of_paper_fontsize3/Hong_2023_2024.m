...
% Collision Alarm
a=load('D:\matlabWorkspace\model_of_paper_fontsize3\results\result_2023_2024.txt');
plot(a(:,1),a(:,2),'-r','LineWidth',6);
hold on;

% a_size = size(a); % a的尺寸
% a_row = a_size(1, 1);
% % 绘制垂直于x轴的线段(x1,0)至(x1,y1)，使用line([x1 x1], [0 y1])画线段;
% for i = 1 : 1 : a_row
%     x1 = i/30;
%     y1 = a(i, 2);
%     %
%     line([x1 x1], [0 y1], 'linestyle', '-', 'Color', 'b', 'LineWidth', 3);
%     
% end

% 绘制垂直于x轴的线段(148, 0)至(148, 1.05)，使用line([148 148], [0 1.05]);
line([148 148], [0 1.05], 'linestyle', '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 5);
...
l =legend(' PLGMD ', 'Collision frame');
